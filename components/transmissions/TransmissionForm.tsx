...
interface TransmissionFormProps {
  initialData?: TransmissionType | null;
}
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const url = initialData
        ? `/api/transmissions/${initialData._id}`
        : "/api/transmissions";
      ...
      if (res.ok) {
        setLoading(false);
        toast.success(`Transmission ${initialData ? "updated" : "created"}`);
    ...
    <div className="h-screen p-10 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 rounded-lg">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Transmission</p>
